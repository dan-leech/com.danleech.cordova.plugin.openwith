...
- (void) checkForFileToShare {
    ...
    [self.userDefaults synchronize];
    NSObject *object = [self.userDefaults objectForKey:@"shareData"];
    if (object == nil) {
    ...
    [self.userDefaults removeObjectForKey:@"shareData"];
    ...
    NSMutableArray *dataArray = (NSMutableArray*)object;
    NSMutableArray *itemsArrayResponse = [NSMutableArray new];
    ...
    for (NSDictionary *dict in dataArray) {
        ...
        int duration = 0;
        if ([type hasPrefix:@"video"] || [type hasPrefix:@"public.movie"]) {
        ...
        [itemsArrayResponse addObject:@{
                                        @"path": path,
                                        @"text": text,
                                        @"type": type,
                                        @"duration": [NSString stringWithFormat:@"%d", duration],
                                        @"utis": utis,
                                        @"uri": uri,
                                        @"name": name
                                        }];
